...
#Raises ValueError if something goes haywire
compoundBoundaryRe=re.compile(ur"<Cmpnd>\+|\+#",re.U)
def analyze_reading(reading):
    if isinstance(reading,str):
        reading=unicode(reading,"utf-8")
    if reading.endswith(u"+?"):
        return reading[:-2],[]
    compoundParts=compoundBoundaryRe.split(reading)
    lemma=[]
    for compoundPart in compoundParts:
        if compoundPart.startswith(u"#") and len(compoundPart)>1:
...
def omorfi_lookup(token):
    ...
    #Handling unknown compounds with hyphen
    if len(readings)==0:
        parts=hyphenRe.split(token)
        if len(parts)>1:
            last=parts[len(parts)-1]
            prefix=u"<Cmpnd>+".join(parts[i] for i in range(0,len(parts)-1))
            lastreadings=[atRe.sub(u"",unicode(r,"utf-8")) for r,score in OmorTransducer.lookup(last.encode("utf-8"))]
            #post-process
            se=omorfi_postprocess(token,lastreadings)
            lastreadings=resolve_readings.main(se)
        ...
        readings=resolve_readings.main(s)
                
    
    return readings

def omorfi_readings(token):
    try:
        rs=list(omorfi_lookup(token))
        if len(rs)==0: return u"<OOL>"
        readings=[]
        for r in rs:
            try:
                lemma,tags=analyze_reading(r)
                tagString=u"".join(u"<"+t+u">" for t in analyze_taglist(tags,RET_LIST) if t)
                readings.append(lemma+tagString)
            except:
                print >> sys.stderr, u"something wrong with analyze_reading",token.encode("utf-8"),r.encode("utf-8")
        rstring=u"&&".join(re for re in readings)
        return rstring
            
    except:
        #Something wrong with lookup
        print >> sys.stderr, u"Something wrong with lookup", token.encode("utf-8")
        return u"<OOL>"
    ...
    if options.interactive:
        while True:
            try:
                ...
                print
                break
            print omorfi_readings(unicode(s,"utf-8"))
            continue
