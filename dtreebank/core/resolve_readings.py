...
try:
    import hfst_lookup
    OmorTransducer=hfst_lookup.Transducer("/opt/omorfi-svn/share/hfst/fi/generation.finntreebank.hfstol")
except:
...
def give_last_one(r):
    ...
    #else: ending=r
    index=ending.find(u"<")
    if index!=-1: ending=ending[index:len(ending)]
    return ending

#Makes all necessary der changes to reading r
derRe=re.compile(ur"(<Der_[A-Za-z]+>)",re.U)
def der_changes(r):
    #regex="(<Der_[A-Za-z]+>)"
    if u"<Der_" in r:
        m=derRe.findall(r)
...
def main(readings):
    if len(readings)>1:
        if isCompound(readings) or isDerivation(readings):
            return sorted(handle_cmpnds_and_ders(readings))
        else: #all readings are normal, return just same list
            return readings
    else: #one reading, just return it
        return readings
